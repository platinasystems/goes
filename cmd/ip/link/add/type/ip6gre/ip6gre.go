...
func (c Command) Main(args ...string) error {
	...
	if s := opt.Parms.ByName["ttl"]; len(s) > 0 {
		...
		info = append(info, nl.Attr{Type: rtnl.IFLA_GRE_TTL,
	...
	if flowinfo != 0 {
		info = append(info, nl.Attr{Type: rtnl.IFLA_GRE_FLOWINFO,
			Value: nl.Uint32Attr(flowinfo)})
	}
	...
	if s := opt.Parms.ByName["encaplimit"]; len(s) > 0 {
		...
		info = append(info, nl.Attr{Type: rtnl.IFLA_GRE_ENCAP_LIMIT,
			Value: nl.Uint8Attr(u8)})
	}
	...
	info = append(info, nl.Attr{Type: rtnl.IFLA_GRE_FLAGS,
		Value: nl.Uint32Attr(greflags)})
