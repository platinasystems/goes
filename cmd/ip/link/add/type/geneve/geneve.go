...
func (Command) Main(args ...string) error {
	...
	info = append(info, nl.Attr{rtnl.IFLA_GENEVE_ID, nl.Uint32Attr(u32)})
	...
	if addr := net.ParseIP(s); addr == nil {
		...
		info = append(info, nl.Attr{rtnl.IFLA_GENEVE_REMOTE,
			nl.BytesAttr(ip4)})
	} else {
		info = append(info, nl.Attr{rtnl.IFLA_GENEVE_REMOTE6,
	...
	if s = opt.Parms.ByName["flowlabel"]; len(s) > 0 {
		...
		info = append(info, nl.Attr{rtnl.IFLA_GENEVE_LABEL,
			nl.Be32Attr(u32)})
	}
	if s = opt.Parms.ByName["dstport"]; len(s) > 0 {
		...
		info = append(info, nl.Attr{rtnl.IFLA_GENEVE_PORT,
			nl.Be16Attr(u16)})
	}
	...
	add.Attrs = append(add.Attrs, nl.Attr{rtnl.IFLA_LINKINFO, nl.Attrs{
		nl.Attr{rtnl.IFLA_INFO_KIND, nl.KstringAttr("geneve")},
