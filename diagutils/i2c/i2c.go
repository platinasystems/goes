...
// Package diagutils/i2c provides a command to access i2c devices.
package i2c
...
func (cmd) String() string { return "i2c" }
...
func (cmd) Main(args ...string) error {
	...
	if err != nil {
		return fmt.Errorf("%s: invalid BUS.ADDR[.REG]: %v",
			args[0], err)
	}
	if w != 0 && w != 8 && w != 16 {
		return fmt.Errorf("%d: invalid R/W width", w)
	}

	if dValid {
		_, err = fmt.Sscanf(args[1], "%x", &d)
		if err != nil {
			return fmt.Errorf("%s: invalid value: %v",
				args[1], err)
		}
	}
	...
	if len(args) > 2 {
		...
		if err != nil {
			return fmt.Errorf("%s: invalid delay: %v", s, err)
		}
	}
