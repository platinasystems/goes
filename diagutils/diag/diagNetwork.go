...
func diagNetwork() {
	...
	fmt.Printf("---------------|-------------------------|----------|----------|----------|----------|------|-----------------------------------\n")
        ...
        */
        ...
        /* diagTest: Host reachable, ping host */
        // tbd: change ip addr to host debian host address
        result = diagPing("192.168.101.128", 10)
        if result {r = "pass"} else {r = "fail"}
        fmt.Printf("%15s|%25s|%10s|%10t|%10t|%10t|%6s|%35s\n","mgmt_network","host_ping","-",result,ping_response_min,ping_response_max,r,"ping host 10x")
