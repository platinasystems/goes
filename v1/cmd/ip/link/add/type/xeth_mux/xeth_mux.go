...
const usage = "ip link add [[name] NAME] link LINK type xeth-mux [encap <vlan|vpls>]"

type Command struct{}

func (Command) String() string { return "xeth-mux" }
...
func (Command) Man() lang.Alt {
	return lang.Alt{
		lang.EnUS: `
SEE ALSO
	ip link add type man xeth-mux || ip link add type xeth-mux -man
...
func (Command) Main(args ...string) error {
	...
	args = opt.Parms.More(args, []string{"encap"})
	...
	if s := opt.Parms.ByName["encap"]; len(s) > 0 {
		...
		switch s {
		case "vlan":
			encap = xeth.EncapVlan
		case "vpls":
			encap = xeth.EncapVpls
		default:
			return fmt.Errorf("encap: %q unknown", s)
		}
		data = append(data, nl.Attr{
			Type:  xeth.MuxIflaEncap,
			Value: nl.Uint8Attr(encap),
		})
	}
