...
func RtMsgPtr(b []byte) *RtMsg {
	if len(b) < nl.SizeofHdr+SizeofRtMsg {
		return nil
	}
	return (*RtMsg)(unsafe.Pointer(&b[nl.SizeofHdr]))
}
...
func (rta *Rta) Write(b []byte) (int, error) {
	i := nl.NLMSG.Align(nl.SizeofHdr + SizeofRtMsg)
	if i >= len(b) {
		nl.IndexAttrByType(rta[:], nl.Empty)
		return 0, nil
	}
	nl.IndexAttrByType(rta[:], b[i:])
	return len(b) - i, nil
}
