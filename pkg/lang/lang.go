...
var Precedence = []LANG{LANG(os.Getenv("LANG")), "", EnUS}

type Alt map[LANG]string

// Returns the preferred language text.
func (alt Alt) String() string {
	for _, pref := range Precedence {
		if s, found := alt[pref]; found {
			return s
		}
	}
	return ""
