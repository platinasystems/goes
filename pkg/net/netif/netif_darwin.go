...
func (nif *NetIf) ExtraIfData(ifdata *unix.IfData) {
	...
	if ifdata.Xmitquota != 0 {
		nif.Extra["xmitquota"] = ifdata.Xmitquota
	}
	if ifdata.Metric != 0 {
	...
	if ifdata.Recvtiming != 0 {
		nif.Extra["recvtiming"] = ifdata.Recvtiming
	}
	if ifdata.Xmittiming != 0 {
		nif.Extra["xmittiming"] = ifdata.Xmittiming
	}
