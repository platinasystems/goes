...
func newHostsFile(vpn string, prefix netip.Prefix) (*hostsFile, error) {
	vpnhosts := filepath.Join(vpn, vpnHostsFileName)
	h := &hostsFile{
		path:   filepath.Join(program.StateHome(), vpnhosts),
		prefix: prefix,
	...
	if err != nil {
		config := filepath.Join(program.ConfigHome(), vpnhosts)
	...
	for sc := bufio.NewScanner(r); sc.Scan(); {
		...
		if !h.prefix.Contains(addr) {
			return h, fmt.Errorf("%s:%v: %w [%v]",
				h.path, addr, ErrRange, h.prefix)
		}
...
func (h *hostsFile) addressed(addr netip.Addr) (string, error) {
	...
	name, ok := h.name[addr]
	if !ok {
		return name, fmt.Errorf("%s[%v]: %w", h.path, addr, ErrNotFound)
	}
	return name, nil
}
...
func (h *hostsFile) named(hn string) (netip.Addr, error) {
	...
	addr, ok := h.addr[hn]
	if !ok {
		err = fmt.Errorf("%s[%s]: %w", h.path, hn, ErrNotFound)
	}
	return addr, err
}
