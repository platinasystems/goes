// Copyright © 2023-2025 Platina Systems, Inc. All rights reserved.
// Use of this source code is governed by the GPL-2 license described in the
// LICENSE file.

//go:build !netlink && (darwin || freebsd || netbsd || openbsd)

package netif

import (
	"context"
	"net"
	"net/netip"
	"unsafe"

	"github.com/platinasystems/goes/v2/pkg/integer"
	"github.com/platinasystems/goes/v2/pkg/xerrors"
	"github.com/platinasystems/goes/v2/pkg/xnet"
	"golang.org/x/sys/unix"
)

type IfMsgHdr = unix.IfMsghdr
type IfData = unix.IfData
type IfaMsgHdr = unix.IfaMsghdr

func Extract[T Msgs](data []byte) (p *T, body, rem []byte) {
	l := xnet.SysctlMsgLen(data)
	p = Pointer[T](data)
	body = data[Sizeof(p):]
	rem = data[xnet.SysctlAlign(l):]
	return
}

func Pointer[T ~uint16 | Msgs](data []byte) *T {
	return (*T)(unsafe.Pointer(&data[0]))
}

func Sizeof[T Msgs](p *T) int {
	return int(unsafe.Sizeof(*p))
}

func List(ctx context.Context) (nifs NetIfs, err error) {
	rib, err := xnet.SysctlGet(
		unix.CTL_NET,
		xnet.AF_ROUTE,
		0,
		xnet.AF_UNSPEC,
		unix.NET_RT_IFLIST,
		0,
	)
	if err != nil {
		return
	}
	nifByIndex := make(map[int]*NetIf)
	for data := rib; len(data) > xnet.SysctlMsgMin; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_IFINFO) {
			data = data[msglen:]
			continue
		}
		im, body, datá := Extract[IfMsgHdr](data)
		data = datá
		nif, ok := nifByIndex[int(im.Index)]
		if !ok {
			nif = &NetIf{
				Extra: make(map[string]any),
			}
			integer.Assign(&nif.Index, im.Index)
			nifs = append(nifs, nif)
			nifByIndex[nif.Index] = nif
		}
		nif.ifinfo(im, body)
	}
	for data := rib; len(data) > 0; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_NEWADDR) {
			data = data[msglen:]
			continue
		}
		ifa, body, datá := Extract[IfaMsgHdr](data)
		data = datá
		if nif, ok := nifByIndex[int(ifa.Index)]; ok {
			nif.ifaddr(ifa, body)
		}
	}
	return
}

func (nif *NetIf) Refresh(ctx context.Context) error {
	if nif.Index == 0 {
		return nif.refreshWithName(ctx)
	}
	return nif.refreshWithIndex(ctx)
}

func (nif *NetIf) refreshWithIndex(ctx context.Context) error {
	rib, err := xnet.SysctlGet(
		unix.CTL_NET,
		xnet.AF_ROUTE,
		int32(nif.Index),
		xnet.AF_UNSPEC,
		unix.NET_RT_IFLIST,
		0,
	)
	if err != nil {
		return err
	}
	for data := rib; len(data) > xnet.SysctlMsgMin; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_IFINFO) {
			data = data[msglen:]
			continue
		}
		im, body, datá := Extract[IfMsgHdr](data)
		data = datá
		if integer.Equal(nif.Index, im.Index) {
			nif.ifinfo(im, body)
		}
	}
	for data := rib; len(data) > 0; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_NEWADDR) {
			data = data[msglen:]
			continue
		}
		ifa, body, datá := Extract[IfaMsgHdr](data)
		data = datá
		if integer.Equal(nif.Index, ifa.Index) {
			nif.ifaddr(ifa, body)
		}
	}
	return nil
}

func (nif *NetIf) refreshWithName(ctx context.Context) error {
	var found bool
	rib, err := xnet.SysctlGet(
		unix.CTL_NET,
		xnet.AF_ROUTE,
		0,
		xnet.AF_UNSPEC,
		unix.NET_RT_IFLIST,
		0,
	)
	if err != nil {
		return err
	}
	for data := rib; len(data) > xnet.SysctlMsgMin; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_IFINFO) {
			data = data[msglen:]
			continue
		}
		im, body, datá := Extract[IfMsgHdr](data)
		data = datá
		dlHdr, dlBody, _ := xnet.SAExtractDataLink(body)
		dlName, _, _ := dlHdr.NAS(dlBody)
		if dlName == nif.Name {
			integer.Assign(&nif.Index, im.Index)
			nif.ifinfo(im, body)
			found = true
			break
		}
	}
	if !found {
		return xerrors.NotFound(nif.Name)
	}
	for data := rib; len(data) > 0; {
		msglen := xnet.SysctlMsgLen(data)
		if len(data) < msglen {
			break
		}
		if !xnet.SysctlMsgOK(data, unix.RTM_NEWADDR) {
			data = data[msglen:]
			continue
		}
		ifa, body, datá := Extract[IfaMsgHdr](data)
		data = datá
		if integer.Equal(ifa.Index, nif.Index) {
			nif.ifaddr(ifa, body)
		}
	}
	return nil
}

func (nif *NetIf) ifinfo(im *IfMsgHdr, body []byte) {
	if (im.Flags & int32(unix.IFF_UP)) != 0 {
		nif.Flags |= net.FlagUp
	}
	if (im.Flags & int32(unix.IFF_BROADCAST)) != 0 {
		nif.Flags |= net.FlagBroadcast
	}
	if (im.Flags & int32(unix.IFF_LOOPBACK)) != 0 {
		nif.Flags |= net.FlagLoopback
	}
	if (im.Flags & int32(unix.IFF_POINTOPOINT)) != 0 {
		nif.Flags |= net.FlagPointToPoint
	}
	if (im.Flags & int32(unix.IFF_MULTICAST)) != 0 {
		nif.Flags |= net.FlagMulticast
	}
	if (im.Flags & int32(unix.IFF_RUNNING)) != 0 {
		nif.Flags |= net.FlagRunning
	}
	integer.Assign(&nif.Type, im.Data.Type)
	integer.Assign(&nif.MTU, im.Data.Mtu)
	integer.Assign(&nif.Rx.Packets, im.Data.Ipackets)
	integer.Assign(&nif.Rx.Bytes, im.Data.Ibytes)
	integer.Assign(&nif.Rx.Errors, im.Data.Ierrors)
	integer.Assign(&nif.Tx.Packets, im.Data.Opackets)
	integer.Assign(&nif.Tx.Bytes, im.Data.Obytes)
	integer.Assign(&nif.Tx.Errors, im.Data.Oerrors)
	integer.Assign(&nif.Collisions, im.Data.Collisions)
	integer.Assign(&nif.Rx.Drops, im.Data.Iqdrops)
	if nif.Extra == nil {
		nif.Extra = make(map[string]any)
	}
	nif.ExtraIfData(&im.Data)
	dlhdr, body, _ := xnet.SAExtractDataLink(body)
	nif.Name, nif.HardwareAddr, _ = dlhdr.NAS(body)
}

func (nif *NetIf) ifaddr(ifa *IfaMsgHdr, body []byte) {
	const min = xnet.SAMin
	var (
		addr netip.Addr
		bits int
	)
	for i := 0; i < unix.RTAX_MAX && len(body) > min; i++ {
		if (int(ifa.Addrs) & (1 << i)) == 0 {
			continue
		}
		sal := xnet.SALen(body)
		if sal > len(body) {
			break
		}
		switch i {
		case unix.RTAX_DST:
			if a := xnet.SAIP(body); a.IsValid() {
				nif.Extra["dst"] = a
			}
		case unix.RTAX_GATEWAY:
			if a := xnet.SAIP(body); a.IsValid() {
				nif.Extra["gw"] = a
			}
		case unix.RTAX_NETMASK:
			if a := xnet.SAIP(body); a.IsValid() {
				bits, _ = net.IPMask(a.AsSlice()).Size()
			}
		case unix.RTAX_IFA:
			if a := xnet.SAIP(body); a.IsValid() {
				addr = a
			}
		case unix.RTAX_AUTHOR:
		case unix.RTAX_BRD:
			if a := xnet.SAIP(body); a.IsValid() {
				nif.Extra["brd"] = a
			}
		}
		body = body[xnet.SysctlAlign(sal):]
	}
	if addr.IsValid() && bits > 0 {
		prefix := netip.PrefixFrom(addr, bits)
		nif.Prefixes = append(nif.Prefixes, prefix)
	}
}
