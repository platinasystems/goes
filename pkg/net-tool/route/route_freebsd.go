...
`
	gwFlags = `{blackhole, proto1, proto2, proto3, reject, xresolve,
static, sticky}`
	gwMetrics = `{mtu, hopcount, expire, recvpipe, sendpipe\n"+
 ssthresh, rtt, rttvar, weight}`
)
...
func rtmmetrics(rtm *unix.RtMsghdr, metrics []string) {
	for _, metric := range metrics {
		...
		case "weight":
			integer.Assign(&rtm.Rmx.Weight, val)
			rtm.Inits |= unix.RTV_WEIGHT
		}
	}
}
