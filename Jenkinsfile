...
pipeline {
	...
	stage('Build') {
		...
		dir('goes') {
		    echo "Building goes..."
		    sh 'export PATH=/usr/local/go/bin:${PATH}; for package in `find . -type d -print` ; do ls $package/*.go > /dev/null 2>&1 && ( echo "Working on" $package ;go build -v $foo || break ) || echo "Skipping" $package;done'
		}
	    }
	}
    }
