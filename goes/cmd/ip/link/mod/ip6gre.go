...
func (m *mod) parseTypeIp6Gre() error {
	...
	if s := m.opt.Parms.ByName["ttl"]; len(s) > 0 {
		...
		m.tinfo = append(m.tinfo, nl.Attr{rtnl.IFLA_GRE_TTL,
			nl.Uint8Attr(u8)})
	}
	...
	if flowinfo != 0 {
		m.tinfo = append(m.tinfo, nl.Attr{rtnl.IFLA_GRE_FLOWINFO,
	...
	if s := m.opt.Parms.ByName["encaplimit"]; len(s) > 0 {
		...
		m.tinfo = append(m.tinfo, nl.Attr{rtnl.IFLA_GRE_ENCAP_LIMIT,
			nl.Uint8Attr(u8)})
	}
	...
	m.tinfo = append(m.tinfo, nl.Attr{rtnl.IFLA_GRE_FLAGS,
