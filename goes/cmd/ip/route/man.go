...
DESCRIPTION
	...
	unicast
		the route entry describes real paths to the destinations
		covered by the route prefix.

	unreachable
		these destinations are unreachable.

		Packets are discarded and the ICMP message host unreachable is
		generated.  The local senders get an EHOSTUNREACH error.

	blackhole
		these destinations are unreachable.

		Packets are discarded silently.  The local senders get an
		EINVAL error.

	prohibit
		these destinations are unreachable.

		Packets are discarded and the ICMP message communication
		administratively pro‐ hibited is generated. The local senders
		get an EACCES error.
	...
	throw
		...
		pretending that no route was found. Without policy routing it
		is equivalent to the absence of the route in the routing table.
		The packets are dropped and the ICMP message net unreachable is
		generated. The local senders get an ENETUNREACH error.

	nat
		a special NAT route.

		Destinations covered by the prefix are considered to be dummy
		(or external) addresses which require translation to real (or
		internal) ones before forward‐ ing. The addresses to translate
		to are selected with the attribute via.  Warning: Route NAT is
		no longer supported in Linux 2.6.

	anycast
		not implemented the destinations are anycast addresses assigned
		to this host.
		
		They are mainly equivalent to local with one difference: such
		addresses are invalid when used as the source address of any
		packet.
	...
	ip route replace
		...
		to TYPE PREFIX (default)
			...
			If TYPE is omitted, ip assumes type unicast.  Other
			values of TYPE are listed above.  PREFIX is an IP or
			IPv6 address optionally fol‐ lowed by a slash and the
			prefix length. If the length of the prefix is missing,
			ip assumes a full-length host route. There is also a
			special PREFIX default - which is equivalent to IP 0/0
			or to IPv6 ::/0.

		tos TOS

		dsfield TOS
			the Type Of Service (TOS) key.
 
			This key has no associated mask and the longest match
			is understood as: First, compare the TOS of the route
			and of the packet. If they are not equal, then the
			packet may still match a route with a zero TOS.  TOS is
			either an 8 bit hexadecimal number or an identifier
		...
		vrf NAME
			the vrf name to add this route to.
 
			Implicitly means the table associated with the VRF.
		...
		via [ FAMILY ] ADDRESS
			...
			Actually, the sense of this field depends on the route
			type.  For normal unicast routes it is either the true
			next hop router or, if it is a direct route installed
			in BSD compatibility mode, it can be a local address of
			the interface. For NAT routes it is the first address
		...
		rtt TIME
			...
			passed directly to the routing code to maintain
			compatibility with previ‐ ous releases.  Otherwise if a
			suffix of s, sec or secs is used to specify seconds and
		...
		rto_min TIME (2.6.23+ only)
			...
			communicating with this destination.
		...
		cwnd NUMBER (2.3.15+ only)
			...
			It is ignored if the lock flag is not used.
		...
		initcwnd NUMBER (2.5.70+ only)
			...
			(Maximal Segment Size) for same connection. The
			default is zero, meaning to use the val‐ ues specified
			in RFC2414.
		...
		initrwnd NUMBER (2.6.33+ only)
			...
			Actual window size is this value multiplied by the MSS
			of the connection.  The default value is zero, meaning
			to use Slow Start value.

		features FEATURES (3.18+only)
			Enable or disable per-route features.
			
			Only available feature at this time is ecn to enable
		...
		congctl lock NAME (3.20+ only)
			...
			algorithm for that des‐ tination. If the modifier lock
			is used, then an applica‐ tion is not allowed to
			overwrite the specified congestion control algorithm
			for that destination, thus it will be
			enforced/guaranteed to use the proposed algorithm.

		advmss NUMBER (2.3.15+ only)
			the MSS ('Maximal Segment Size') to advertise to these
			destinations when establishing TCP connections.
			
			If it is not given, Linux uses a default value
			calculated from the first hop device MTU.  (If the path
		...
		nexthop NEXTHOP
			...
			NEXTHOP is a complex value with its own syntax similar
			to the top level argu‐ ment lists:
		...
		scope SCOPE_VAL
			...
			ip assumes scope global for all gatewayed unicast
			routes, scope link for direct unicast and broadcast
		...
		protocol RTPROTO
			...
			not given, ip assumes protocol boot (i.e. it assumes
			...
			redirect
				the route was installed due to an ICMP
				redirect.
			...
			boot
				the route was installed during the bootup
				sequence.  If a routing daemon starts, it will
				purge all of them.

			static
				the route was installed by the administrator to
				override dynamic routing. Routing dae‐ mon will
				respect them and, probably, even adver‐ tise
				them to its peers.
		...
		onlink
			pretend that the nexthop is directly attached to this
			link, even if it does not match any interface prefix.

		pref PREF
			the IPv6 route preference.
			
			PREF is a string specifying the route preference as
			defined in RFC4191 for Router Discovery messages.
			Namely:
		...
		encap ENCAPTYPE ENCAPHDR
			...
			ip id TUNNEL_ID dst REMOTE_IP [ tos TOS ] [ ttl TTL ]
	...
	ip route show
		...
		table TABLEID
			...
			The default setting is to show table main.  TABLEID may
			either be the ID of a real table or one of the special
			values:
		...
		from SELECTOR
			...
			address range rather than destinations.
			
			Note that the from option only works with cloned
			routes.
	...
	ip route get
		...
		pretends that a packet arrived from this interface and searches
		for a path to forward the packet.
...
EXAMPLES
	...
	ip route add 10.1.1.0/30 encap mpls 200/300 via 10.1.1.1 dev eth0
		Adds an ipv4 route with mpls encapsulation attributes attached
		to it.
