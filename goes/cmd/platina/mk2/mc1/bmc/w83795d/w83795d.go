...
// Package w83795d provides access to the H/W Monitor chip at the Fan tray of CH1
...
func (c *Command) Main(...string) error {
	...
	if c.rpc, err = atsock.NewRpcServer(Name); err != nil {
		return err
	}

	rpc.Register(&c.Info)
	for _, v := range WrRegDv {
		err = redis.Assign(machine.Name+":"+v+".", Name, "Info")
		if err != nil {
			return err
		}
	}
...
func (c *Command) update() error {
	...
	for j := 0; j <= MaxFanTrays; j++ {
		...
		k = "fan_tray." + strconv.Itoa(j+1) + ".hwmon.front.temp.units.C"
		v, err = Vdev[j].FrontTemp()
		if err != nil {
			return err
		}
		if v != c.lasts[k] {
		...
		k = "fan_tray." + strconv.Itoa(j+1) + ".hwmon.rear.temp.units.C"
		v, err = Vdev[j].RearTemp()
		if err != nil {
			return err
		}
		if v != c.lasts[k] {
		...
		k = "fan_tray." + strconv.Itoa(j+1) + ".hwmon.temp.target.units.C"
		x, err := Vdev[j].GetHwmTarget()
		if err != nil {
			return err
		}
		if x != c.last[k] {
...
func (h *I2cDev) FanCount(t uint8, i uint8) (uint16, error) {
	var rpm uint16
	...
	w := "fan_tray." + strconv.Itoa(int(t)) + ".status"
