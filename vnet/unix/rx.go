...
func (n *rx_node) init(m *Main) {
	...
	n.pv_pool = make(chan *rx_packet_vector, 4*vnet.MaxVectorLen)
	n.rv_pool = make(chan *rx_ref_vector, 4*vnet.MaxVectorLen)
	n.rv_input = make(chan *rx_ref_vector, 4*vnet.MaxVectorLen)
