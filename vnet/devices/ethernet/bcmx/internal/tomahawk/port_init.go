...
import (
	...
	"github.com/platinasystems/vnetdevices/ethernet/switch/bcm/internal/cmic"
	"github.com/platinasystems/vnetdevices/ethernet/switch/bcm/internal/m"
	"github.com/platinasystems/vnetdevices/ethernet/switch/bcm/internal/packet"
	...
	"github.com/platinasystems/vnetdevices/ethernet/switch/bcm/internal/tsc"
...
func (p *Port) GetHwInterfaceCounterNames() vnet.InterfaceCounterNames {
	return p.Switch.(*tomahawk).InterfaceCounterNames
}
...
func (t *tomahawk) addPort(p *Port, pb *port.PortBlock) {
	c := t.GetSwitchCommon()
...
func (t *tomahawk) addCpuLoopbackPort(p *Port, name string, pn physical_port_number, speed vnet.Bandwidth) {
	v := t.Vnet
	p.Switch = t
...
func (t *tomahawk) addCpuLoopbackPorts() {
	t.addCpuLoopbackPort(&t.cpuPort, "bcm-cpu", phys_port_cpu, 10e9)
	for pipe := uint(0); pipe < n_pipe; pipe++ {
		t.addCpuLoopbackPort(&t.loopbackPorts[pipe],
			fmt.Sprintf("bcm-pipe%d-loopback", pipe),
			phys_port_loopback_for_pipe(pipe),
			10e9)
	}
}

func (t *tomahawk) PortInit(vn *vnet.Vnet) {
	port.Init(vn)
	tsc.Init(vn)
	t.cliInit()
...
func (t *tomahawk) set_port_bitmaps() {
...
func (t *tomahawk) linkScanAdd(p *Port, pb *port.PortBlock) {
...
func (t *tomahawk) setLedPortState(p physical_port_number, bit uint8, isSet bool) {
	var li uint
...
func (t *tomahawk) LinkStatusChange(v *cmic.LinkStatus) {
	n := uint16(0)
	for i := range v {
...
func (t *tomahawk) enablePortBlock(pb *port.PortBlock, enable bool) {
...
func (t *tomahawk) enablePorts(enable bool) {
...
func (t *tomahawk) port_mapping_init() {
	q := t.getDmaReq()
