...
import (
	...
	"github.com/platinasystems/go/vnet/devices/ethernet/switch/bcm/internal/pipemem"
...
//go:generate gentemplate -d Package=tomahawk -id l3_interface -d VecType=l3_interface_vec -d Type=l3_interface github.com/platinasystems/go/elib/vec.tmpl
...
func (t *tomahawk) l3_interface_init() {
...
func (p *Port) GetSwInterfaceCounterNames() (nm vnet.InterfaceCounterNames) {
	...
	nm.Combined = []string{
		bcm_rx_if_counter: rx_counter_prefix + "l3 interface",
	}
	return
}

func (t *tomahawk) swIfCounterSync(v *vnet.Vnet) {
	...
	p.Pool.Foreach(func(pipe, index uint) {
		...
		if c.value.Packets != 0 {
			kind := v.HwSwCombinedIfCounter(bcm_rx_if_counter)
...
func (t *tomahawk) l3_interface_for_si(si vnet.Si) *l3_interface {
	return &t.l3_interface_main.if_by_si[si]
}

func (t *tomahawk) swIfAddDel(v *vnet.Vnet, si vnet.Si, isDel bool) (err error) {
	// si not sw interface derived from tomahawk port?
