...
func (p physical_port_number) speedBitsPerSec(t *tomahawk) float64 {
...
func (t *tomahawk) set_mmu_pipe_map() {
	c := t.GetSwitchCommon()
	cf := &c.SwitchConfig
...
func (p physical_port_number) toMmu(t *tomahawk) (i idb_mmu_port_number, m mmu_pipe) {
	i, m = idb_by_phys[p], t.mmu_pipe_by_phys_port[p]
	i = i.gratuitous_mmu_port_scrambling()
	return
}

func (p physical_port_number) toGlobalMmu(t *tomahawk) mmu_global_port_number {
	i, pipe := p.toMmu(t)
	if pipe != ^mmu_pipe(0) {
