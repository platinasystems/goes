...
func (t *fe1a) flex_counter_init_offset_table(b sbus.Block, pool uint) {
	q := t.getDmaReq()
	p0, p1 := pool/4, pool%4
...
func (t *fe1a) flex_counter_fifo_dma_start() {
	fm := &t.flex_counter_main
...
//go:generate gentemplate -d Package=fe1a -id flex_counter -d VecType=flex_counter_vec -d Type=flex_counter github.com/platinasystems/go/elib/vec.tmpl
...
func (x *flex_counter_ref_entry) alloc(t *fe1a, poolIndex, pipeMask uint, b sbus.Block) (ref pipemem.Ref, ok bool) {
	fm := &t.flex_counter_main
	pm := fm.get_pipe(b)
...
func (x *flex_counter_ref_entry) free(t *fe1a, b sbus.Block) {
	fm := &t.flex_counter_main
	pm := fm.get_pipe(b)
...
func (x *flex_counter_ref_entry) get_value(t *fe1a, b sbus.Block) (v vnet.CombinedCounter) {
	fm := &t.flex_counter_main
	pm := fm.get_pipe(b)
	pool := &pm.pools[x.pool]
	i, m := x.ref.Get()
...
func (x *flex_counter_ref_entry) update_value(t *fe1a, pipe uint, b sbus.Block) (v flex_counter_entry) {
	fm := &t.flex_counter_main
	pm := fm.get_pipe(b)
	pool := &pm.pools[x.pool]
...
// 	}
// }
func (e *flex_counter_eviction_fifo_elt) tx_pipe_perq(t *fe1a) {
	i := e.index
