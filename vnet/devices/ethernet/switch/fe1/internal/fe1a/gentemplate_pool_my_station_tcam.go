...
// generated from gentemplate [gentemplate -d Package=fe1a -id my_station_tcam -d PoolType=my_station_tcam_pool -d Type=my_station_tcam_entry -d Data=entries github.com/platinasystems/go/elib/pool.tmpl]
...
func (p *my_station_tcam_pool) ForeachIndex(f func(i uint)) {
	for i := range p.entries {
		if !p.Pool.IsFree(uint(i)) {
			f(uint(i))
		}
